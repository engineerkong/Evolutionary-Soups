...
def build_dataset_eval(path, tokenizer, rm_tokenizers_list, split='test', size=None):
    ...
    ds = ds.select(range(min(5, len(ds)))) # EDITED for quick eval
...
def merge_weights_with_preference(base_model_names, preference, temp_save_path):
    ...
    for base_model_name in base_model_names:
        peft_config = PeftConfig.from_pretrained(base_model_name)
        # Load the base model
        ...
        # Now load the adapter to get the fine-tuned model
        model_tmp.resize_token_embeddings(32001)
        model_tmp = PeftModel.from_pretrained(model_tmp, base_model_name)
        model_tmp.resize_token_embeddings(32000)
