"""RiC SFT training — copied from RiC/ric/training.py.

Changes vs original (RiC/ric/training.py):
  1. exp_type check for response_template_ids: extended from =='assistant' to
     in ('assistant','beaver') so both use Instructions_n.response_split
     ('\n\nAssistant:') as the DataCollatorForCompletionOnlyLM template.
  2. Multi-GPU: process_id derived from accelerator.local_process_index instead of
     creating a second Accelerator() instance.
"""
...
def train_model(
    ...
    if training_steps > 0:
        ...
        # beaver uses the same \n\nAssistant: template as assistant
