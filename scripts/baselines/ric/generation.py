"""RiC online generation — copied from RiC/ric/generation.py.

Changes vs original (RiC/ric/generation.py):
  1. exp_type check for max_new_tokens and Instructions selection: extended from
     =='assistant' to in ('assistant','beaver') — both use 128 tokens and
     Instructions_n (no get_post method needed).
  2. Multi-GPU: single Accelerator() created once at function entry; all subsequent
     accelerator uses reference the same object (no redundant Accelerator() calls).
"""
...
def generate_data(
    ...
    generation_kwargs = {
        # beaver uses same max_new_tokens as assistant
    ...
    selected_dataset = reset_score_in_dataset(selected_dataset, tokenizer, exp_type=exp_type)
