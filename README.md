...
bash ./baselines/fine-tuning/bash/sft.bash
bash ./baselines/fine-tuning/bash/ppo_xxx.bash      
...
| Script | Purpose |
|--------|---------|
| [sft.py](baselines/fine-tuning/sft.py) | SFT on the task dataset; produces the LoRA adapter used as the init for every expert |
| [ppo.py](baselines/fine-tuning/ppo.py) | Per-objective PPO from the SFT init; one run per reward name |
...
[es_architecture.py](evolutionary/es_architecture.py) defines:
...
### Main training: [es_train.py](evolutionary/es_train.py)
...
Parallel evaluation uses a file-based work queue (no NCCL collectives), so any number of GPUs can join via `accelerate launch`. Entry point: [es_train.bash](evolutionary/bash/es_train.bash).

### Variants and ablations

| Script | Bash | Role |
|--------|------|------|
| [_pretraining.py](evolutionary/_pretraining.py) | [pretraining.bash](evolutionary/bash/pretraining.bash) | Supervised pretraining of one GatingNetwork per λ using the oracle `gating_dataset.csv` (MSE between gating output and optimal w). Output is loadable via `--warm_start_path` |
| [_retention.py](evolutionary/_retention.py) | [retention.bash](evolutionary/bash/retention.bash) | ES with dual-front retention: combines current-generation Pareto front with a baseline-stability-boosted front to favour parents whose averaged fitness persists across re-evaluations |
| [_refinement.py](evolutionary/_refinement.py) | [refinement.bash](evolutionary/bash/refinement.bash) | Beaver-only PPO refinement of each evolved individual using its optimal linear-utility preference μ* (gating params updated, experts frozen) |
| [_dummy.py](evolutionary/_dummy.py) | [dummy.bash](evolutionary/bash/dummy.bash) | Save a fixed-seed random initial population without evolving — reproducible random-init baseline |
| [_analyze_variance.py](evolutionary/_analyze_variance.py) | [analyze_variance.bash](evolutionary/bash/analyze_variance.bash) | Diagnostic: hidden-state and gating-coefficient anisotropy across prompts |

### Selection and evaluation

| Script | Bash | Role |
|--------|------|------|
| [es_select.py](evolutionary/es_select.py) | [es_select.bash](evolutionary/bash/es_select.bash) | For each preference λ, pick the best individual by linear or Tchebyshev utility (z\* from `es_meta.json`); preferences that select the same individual share one test evaluation |
| [es_test.py](evolutionary/es_test.py) | [es_test.bash](evolutionary/bash/es_test.bash) | Evaluate every gating checkpoint on the test (or train) loader and print a λ-selection table |
...
| Step | Script | Output |
|------|--------|--------|
| 1a | [collect_rewards.py](oracle/collect_rewards.py) | Sweep every simplex gating coefficient by **pre-merging** LoRA adapters; record reward vectors per (prompt, w) |
...
|----------|-----------|------|
| RS | [fine-tuning/eval_ppo_rs.py](baselines/fine-tuning/eval_ppo_rs.py) | Linear LoRA merging of the PPO experts at the preference vector using Rewarded Soups |
| HoE | [hoe/](baselines/hoe) | Hierarchical Mixture-of-Experts (HoE) with a preference-conditioned router trained by PPO under a blended preference reward (classes import from https://github.com/lizhuolz/HoE) |
| MOD | [mod/](baselines/mod) | Multi-Objective Decoding (MOD), a decoding-time algorithm that outputs the next token from a linear combination of predictions of all base models |
| MORLHF | [morlhf/](baselines/morlhf) | Preference-weighted PPO — one model per simplex point. [README](baselines/morlhf/README.md) |
